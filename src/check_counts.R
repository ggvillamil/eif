...
deseq_top <- deseq_res %>% as.data.frame() %>%
  ...
  rename(log2FoldChange_ribo = "log2FoldChange") %>%
  rename(padj_ribo = "padj")

# Get transcript IDs of top 15
deseq_top_tx <- rownames(deseq_top)

deseq_res_rna <- deseq_res_rna %>% as.data.frame() %>%
  rename(log2FoldChange_rna = "log2FoldChange") %>%
  rename(padj_rna = "padj") %>%
  select(log2FoldChange_rna, padj_rna)
...
deseq_top_tx <- c(deseq_top_tx)

compare_original <- deseq_top[deseq_top_tx,] %>% as.data.frame() %>% select(log2FoldChange_ribo, padj_ribo) %>% cbind(deseq_res_rna[deseq_top_tx,]) %>% cbind(counts_original[deseq_top_tx,]) %>% rownames_to_column("transcript_id")
...
write_tsv(compare_original, "results/post/table_multimap_check_top_15_original_markusfilter.tsv")
